// netlify/functions/secretary-stream.mjs
//
// 秘書Zoe(mode:"secretary")専用の、ストリーミング配信版エンドポイントです。
// 以前はchat-stream.mjsの中でchat.jsをimportして共用していましたが、
// 「Load failed」という原因不明の通信エラーが解決しなかったため、
// chat.jsへの依存を一切なくし、このファイル単体で完結する形に作り直しました。
// (設定Zoe用のchat-stream.mjsとは完全に独立しています)
//
// プロンプト・ツール定義・実行ロジックは、動作実績のあるchat.js内の
// 秘書Zoe関連コードをそのまま複製したものです(内容の変更はありません)。
// 今後、秘書Zoeの仕様を変更する場合は、chat.js側とこのファイルの
// 両方を同じ内容に保つよう注意してください(二重管理になっています)。

import { getStore } from "@netlify/blobs";

const RATE_LIMIT_STORE_NAME = "rate-limit-chat";
...
async function checkShortRateLimit(ip) {
  try {
    const store = getStore({
...
// ②content配列(tool_use含む)を組み立てて呼び出し元に返す
...
export default async (req, context) => {
  ...
  try {
    ...
    const withinLimit = await checkShortRateLimit(clientIp);
    if (!withinLimit) {
        ...
        try {
          ...
          for (let i = 0; i < 8; i++) {
            ...
            for (const block of toolUseBlocks) {
              ...
              const toolResultText = await executeSecretaryTool(block.name, block.input, clientIp);
    ...
    // ここに来る場合は、ストリーム開始前の想定外エラー(バグ)。
    // 「Load failed」のような無応答ではなく、必ず内容の分かるJSONエラーを返す
    return new Response(JSON.stringify({ error: "サーバー内部エラー: " + err.message }), {
      status: 500,
      headers: { ...corsHeaders, "Content-Type": "application/json" },
    });
  }
...
export const config = {
  path: "/.netlify/functions/secretary-stream",
};
