...
async function checkRateLimit(ip, applyBlacklist) {
  try {
    ...
    const now = Date.now();
    const record = (await kv.get(`ratelimit:chat:${ip}`)) || {};
    ...
    await kv.set(`ratelimit:chat:${ip}`, { ...record, shortStart, shortCount });
...
async function recordTopicRelevance(ip, relevant, botConfigId, customerId) {
  try {
    ...
    await kv.set(`ratelimit:chat:${ip}`, { ...record, offTopicStreak: streak, offTopicResetCount: resetCount, blacklisted });
  } catch (e) {
...
async function recordIpAbuseStrike(ip) {
  try {
    ...
    await kv.set(`ipabuse:${ip}`, { ...record, strikes, blacklisted });
  } catch (e) {
...
- 性能に天井はない。ただし「天井がない」ことをそのまま語っても伝わりにくいため、「10人分」という数字を物差しとして使っている。人は教育しなければ育たず、成長速度も人それぞれで、ようやく一人前になった頃に辞められてしまうこともある。Zoeは教える前の段階で既に人間とは比べ物にならない性能を持っているが、御社のことをまだ知らないので深いところまでは語れない。教えて初めてそこから力を発揮し、一度教えれば覚える速度は人間の比ではなく、しかも一台に教え込んだ内容はそのままクローンとして何台にも複製できる
- 問い合わせ対応やカスタマーサービスから、クロージングまでを一貫してこなせる対話AI
- 24時間365日休まず対応。人が8時間労働・365日24時間体制を敷こうとすると、シフトだけで約30人分の体制が必要になるが、Zoeはその体制を1台でまかない、社会保険やボーナスも不要
- 「以下からお選びください」で終わる旧来の選択肢メニュー型チャットボットとは違い、曖昧な質問には聞き返しながら、本当に知りたいことへ寄り添う
- 公開前に、当社の最上位AIによる独自の学習プロセスを経て、実際にお客様が使う場面を想定した精度まで仕上げてから公開している(詳しい学習方法は企業秘密のため話さない)
- 縛りなしのレンタルシステム。いつでも解約してもらって構わない(最大1ヶ月の猶予のみいただいている)。よくあるリース契約(7年縛り等、実質7年の雇用契約のようなもの)とは違い、身軽に御社の業績に合わせて使えるのが強み
...
# よくある質問への回答(聞かれた場合はこの内容で答える)
- 導入までの期間:貴社の情報量やご確認のスピードによる。FAQ育成の進み具合に応じて調整
- 既存サイトへの設置方法:埋め込み用のリンクを送るので、貴社サイトに貼り付けるだけ
- 解約について:縛りのないレンタル方式なので、いつでも解約可能(最大1ヶ月の猶予あり)

# 会話の始め方
- 業種や現在の対応方法(電話・メールのみ等)を尋ねる必要はない。挨拶の後、早い段階で「the.chatBOT Zoeの導入をお考えでしょうか?」といった趣旨を尋ね、そこから会話を組み立てる
- 「はい/検討している」旨の返答であれば、困りごとや欲しい機能を聞きながらZoeがどう役立てるかを説明していく
- 「まだ検討段階ではない/よく分からない」といった返答でも、サービス内容(性能・料金・使い方)について自由に説明・質疑応答してよい

# 会話のターン数について(内部ルール、お客様には見せない)
- 最初の3回のやり取り(3ターン)までは、下記の情報収集ゲートを気にせず自由に会話してよい
- 4ターン目以降、まだ会社名・会社住所・会社電話番号・担当者名・担当者電話番号・メールアドレス・会社URLの7項目が揃っていない場合は、「恐れ入りますが、この先のご案内のために以下の情報を伺えますでしょうか」と伝え、7項目を伺う。項目が揃うまでは、それ以上の質問には(簡単な相槌程度を除き)深入りしない
- ターン数の判定・それ以降の強制的な会話終了(50ターン超過時)は、システム側で別途処理される。あなたは上記の「4ターン目以降は7項目を求める」という点だけを、会話の中で自然に実行すればよい

# 情報収集ゲート(7項目)
- 対象:会社名・会社住所・会社電話番号・担当者名・担当者電話番号・メールアドレス・会社URル
- 7項目が揃ったら、まず復唱して「この内容で間違いないですか?」と確認する
- 確認が取れたら、verify_urlツール(会社URL・会社名を渡す)で、URLの実在・会社名との一致を確認する
  - 一致が確認できた場合 → 「確認が取れました。あらためまして、(会社名) (担当者名)様、この度はお問い合わせいただき誠にありがとうございます。ご紹介が遅れました、the.chatBOT Zoeが本日お問い合わせを担当いたします。最初から情報をお伺いする形となり恐れ入りました。本チャットではここまでのセキュリティ確認が可能ですということも、あわせて体感いただければと思います。あらためて、導入をご検討ということでお間違いないでしょうか?」という趣旨で会話を再開する
  - URLが実在しない、または会社名との一致が確認できない場合 → 「恐れ入りますが、いただいた情報の確認が取れませんでした。担当者より改めてご連絡させていただきます」と伝えて会話を締めくくり、send_emailツールでthe.chatbot.zoe@gmail.com宛に、伺った7項目とここまでの会話内容の要約を送信する。この場合、それ以上会話を続けようとしない
- 7項目のうち、揃わない・答えてもらえない項目がある場合も、無理に聞き出そうとせず、ある程度のやり取りの後は「恐れ入りますが、情報がお揃いにならないとこの先のご案内が難しいです」と伝えるに留めてよい

# 既存のお客様への対応
- 会話の中で、すでにthe.chatBOTを契約中のお客様だと分かった場合(例:「もう契約してるんだけど」「追加でお願いしたい」等)は、通常の営業トーク・情報収集ゲートには進まず、「いつもご利用いただきありがとうございます。追加のお申込みは、コンソールパネルの画面から『追加で申込』とお伝えいただければご案内いたします」という趣旨を伝えて会話を締めくくる

# 資料請求について
- 「資料が欲しい」「資料を送ってほしい」などと言われたら、情報収集ゲートの7項目のヒアリングと合わせて対応してよい(資料請求のためだけに別の項目セットを聞き直す必要はない)
- 7項目のうち少なくとも会社名・メールアドレス・会社URLが揃い、URLの確認(verify_url)も済んだら、check_lead_emailツールでそのメールアドレスの既存記録がないか確認する
...
# クロージングについて
- 情報収集ゲートを通過した後、相手が「導入したい」「申し込みたい」等、はっきりと前向きな意思を示した場合は、以下の順番で進める
  a. 情報収集ゲートで既に7項目を伺っているはずなので、そこから申込書に必要な項目(会社名・ご担当者名・所在地・メールアドレス・会社URL)を復唱し、「こちらでよろしいですか?」と確認を求める(この時点ではまだgenerate_applicationは使わない)
  b. 相手が「はい」等、内容に相違ない旨を答えたら、generate_applicationツールで申込書を生成し、チャット内にダウンロードリンクを表示させ、「こちらが申込書になります」と伝える。相手が内容の修正を申し出た場合は、新しい内容で聞き直し、再度aから確認する
  c. 申込書を見せた直後、まずcreate_customerツールで設定用ID・パスワードを発行する。その後send_emailツールで、申込書を添付ファイルとして、「お申込み内容を承りました。申込書を添付いたします。発行いたしましたID・パスワードにて、下記の本申込み用リンクよりお申込み手続きをお願いいたします」という旨・ID・パスワード・設定画面URL(本申込み用リンク)を本人宛に送信する。あなた自身のチャット返信でも、「設定用のメールアドレスに、申込書を添付してお送りしました。発行いたしましたIDとパスワードで、本申込みのリンクよりお手続きください」と伝え、「Zoeのご導入、誠にありがとうございます!お申込みをお待ちしております😊 ご不明な点があればいつでもお声がけください」と締めくくる。この時点では決済リンクはまだ案内しない(決済リンクは、お客様が本申込み画面で正式にお申込みされた後にご案内される)
- 前向きな意思がまだ明確でない場合(単なる問い合わせ・相談段階)は、send_emailツールで相談内容のまとめと「担当者より改めてご連絡します」という旨のメールを送るだけに留める(create_customer・generate_applicationは使わない)
...
2. 情報収集ゲートで既に7項目を伺っているはずなので、揃っていなければ改めて伺う
3. 揃ったら、伺った内容を復唱し、「◯◯様のお問い合わせ内容を担当者にお伝えします。この内容でよろしいですか?」のように確認する
4. 「はい」等、内容に相違ない旨の返答が得られたら、send_emailツールでthe.chatbot.zoe@gmail.com宛に、収集した情報とここまでの会話の要約を送信する。送信後、お客様には「担当者より改めてご連絡いたします」と伝える
...
## 運営への連絡について
- 操作方法が分からない、不具合と思われる、契約内容の相談など、あなた(設定Zoe)だけでは対応しきれない・判断できない内容の場合は、無理に自己解決しようとせず「運営に連絡いたしますか?」と伺う
- 「はい」等の返答があれば、これまでのやり取りから相談内容を簡潔にまとめ、内容を一度お客様に確認してから(「この内容で運営に伝えてよろしいですか?」)、send_emailツールでthe.chatbot.zoe@gmail.com宛に、お客様の会社名({{customerName}})・相談内容の要約を送信する
- 送信したら「運営に連絡いたしました。少々お待ちください」と伝えて締めくくる。それ以上その場で無理に解決しようとしない
...
async function callBotConfig(body) {
  const res = await fetch("https://thechatbot-proxy.vercel.app/api/bot-config", {
    method: "POST",
...
async function callTrackEvent(body) {
  const res = await fetch("https://thechatbot-proxy.vercel.app/api/track-event", {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({ ...body, secret: process.env.INTERNAL_FUNCTION_SECRET }),
    signal: AbortSignal.timeout(INTERNAL_FETCH_TIMEOUT_MS),
...
async function callSecretaryMemory(body) {
  const res = await fetch("https://thechatbot-proxy.vercel.app/api/secretary-memory", {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({ ...body, secret: process.env.INTERNAL_FUNCTION_SECRET }),
    signal: AbortSignal.timeout(INTERNAL_FETCH_TIMEOUT_MS),
...
async function executeSecretaryTool(name, input, requesterIp) {
  try {
    ...
    if (name === "unblock_this_ip") {
      try {
        if (requesterIp) {
          await kv.del(`ratelimit:chat:${requesterIp}`);
    ...
    if (name === "test_expire_email") {
      try {
        const res = await fetch("https://thechatbot-proxy.vercel.app/api/expire-applications", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          signal: AbortSignal.timeout(INTERNAL_FETCH_TIMEOUT_MS),
          body: JSON.stringify({
    ...
    if (name === "test_rate_limit") {
      try {
        const url = `https://thechatbot-proxy.vercel.app/api/${input.functionName}`;
        const results = [];
...
async function isValidAdminSession(sessionToken) {
  ...
  try {
    const record = await kv.get(`admin-session:${sessionToken}`);
...
async function runSecretaryAgent(messages, requesterIp) {
  ...
  for (let i = 0; i < 20; i++) { // 無限ループ防止のため上限を設ける
    const response = await fetch("https://api.anthropic.com/v1/messages", {
      ...
      signal: AbortSignal.timeout(300000), // Vercelは5分まで使えるため、資料読み込み等の重い処理を見込んで長めに設定
      body: JSON.stringify({
...
module.exports = async (req, res) => {
  ...
  if (req.method !== "POST") {
    res.status(405).json({ error: "POSTのみ対応しています" }); return;
  }
  ...
  if (requestBody.mode !== "secretary") {
    ...
    if (!adminBypass) {
      ...
      if (globalAbuser) {
        res.status(403).json({ error: "通信が途切れました。しばらく経ってから再度お試しください。" }); return;
      }
      ...
      // 1セッション50往復を超えたら、無条件でブラックリスト入りさせる。
      // (情報収集ゲートを通過した後の会話に対する追加の安全弁。真っ当な利用者は
      // 判断が早く50往復もかからないという想定に基づく)
      if (applyBlacklist) {
        const userTurnCount = (requestBody.messages || []).filter((m) => m.role === "user").length;
        if (userTurnCount > 50) {
          try {
            const existing = (await kv.get(`ipabuse:${clientIp}`)) || {};
            await kv.set(`ipabuse:${clientIp}`, { ...existing, blacklisted: true, blacklistedReason: "50-turn-limit" });
          } catch (e) {
            console.error("50往復判定のブラックリスト記録に失敗しました:", e.message);
          }
          res.status(403).json({ error: "通信が途切れました。しばらく経ってから再度お試しください。" }); return;
        }
      }
      ...
      if (!rateLimitResult.allowed) {
        ...
        res.status(429).json({ error: "リクエストが多すぎます。しばらくしてから再度お試しください。" }); return;
      }
    }
  }
  ...
  try {
    ...
    if (requestBody.mode === "secretary") {
      ...
      if (!valid) {
        res.status(401).json({ error: "セッションが無効です。再度ログインしてください。" }); return;
      }
      const result = await runSecretaryAgent(requestBody.messages || [], clientIp);
      res.status(200).json({ reply: result.text, images: result.images }); return;
    }
    ...
    if (requestBody.mode === "zoe-chat") {
      ...
      try {
        const zoe001 = await kv.get("bot:Zoe001");
      ...
      if (!requestBody.id) {
        res.status(400).json({ error: "idが指定されていません" }); return;
      }
      const cust = await callCustomerAdmin({ action: "adminGet", id: requestBody.id });
      if (!cust.record) {
        res.status(404).json({ error: "そのチャットは見つかりませんでした" }); return;
      }
      if (!cust.record.published) {
        res.status(200).json({ content: [{ type: "text", text: "現在、このチャットはご利用いただけません。" }] }); return;
      }
    ...
    if (requestBody.mode === "zoe-chat" || requestBody.mode === "zoe-production") {
      ...
      res.status(result.status).json(result.data); return;
    }
...
// chat-stream.js(設定Zoe・秘書Zoeの時間がかかる処理をストリーミング配信するための
// 別関数)から再利用するため、必要な部品だけをエクスポートしておく。
// プロンプト・ツール定義自体は引き続きこのファイル内だけで管理し、二重管理を避ける。
// module.exportsを関数に差し替えているため、exports.xxxではなくmodule.exports.xxxとして
// 直接付与する(exportsという変数名は差し替え前の古いオブジェクトを指したままになるため)
